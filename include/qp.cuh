...
//
public:
    __host__ __device__ QP(double *H,double *f,int n,double *A,double *b,int nineq)
    {
        this->n = n;
        this->H = H;
        this->f = f;
    ...
    //
    __host__ __device__ QP(double *H,double *f,int n,double *A,double *b,int nineq,double *Aeq,double *beq,double neq)
    {
        this->n = n;
        this->H = H;
        this->f = f;
    ...
    //
    __host__ __device__ void solve(double *sol)
    {
        if(this->neq == 0)
        {
            int size = this->n*this->n;
            double *y = (double*)malloc(this->n*sizeof(double));
            double *x = (double*)malloc(size*sizeof(double));
            qptols(H, f, y, x, this->n);
            LS *ls = new LS(y,x,this->n,this->n,this->A,this->b,this->nineq);
            ls->lsineq(sol);
            free(y);
            free(x);
            delete ls;
            ...
            double *x = (double*)malloc(size*sizeof(double));
            qptols(H, f, y, x, this->n);
            LS *ls = new LS(y,x,this->n,this->n,this->A,this->b,this->nineq,this->Aeq,this->beq,this->neq);
            ls->lsineqeq(sol);
            free(y);
            free(x);
            delete ls;
...
private:
    //
    __host__ __device__ void qptols(double *H,double *f,double *y,double *x,int n)
    {
        int size = n*n;
        chol(H,n);
    ...
    //
    __host__ __device__ void chol(double *A,int n)
    {
    ...
    //
    __host__ __device__ int Choleski_LU_Decomposition(double *A, int n)
    {
        int i, k, p;
    ...
    //
    __host__ __device__ int Lower_Triangular_Solve(double *L, double B[], double x[], int n)
    {
        int i, k;
